...
\begin{document}
	...
	As discussed in \S\ref{sec:related:interventions}, facilitation interventions can be defined in two ways; whether a comment is facilitative in nature (Def.~\ref{def:facilitative}), or whether a dedicated facilitator would participate at a given point in time (Def.~\ref{def:facilitator}). In this section, we investigate whether we can use available datasets to train and evaluate models using the latter definition. See App.~\ref{sec:appendix:models:llms:facilitator} for experiments focusing on the broader understanding of facilitation by LLMs and App.~\ref{sec:appendix:models:llms:ablations} for ablation studies on both self-consistency and instruction-sensitivity.
	...
	\paragraph{Deciding when NOT to intervene is easy.}
	...
	\label{sec:appendix:models:llms:experimental}
	...
	\subsubsection{Can LLMs understand what facilitation is?}
	\label{sec:appendix:models:llms:facilitator}
	...
	\begin{table*}[t]
		...
		\begin{tabular}{llcccc}
			\toprule
			\textbf{Run A} & \textbf{Run B} & \textbf{OLMo-32B} & \textbf{OLMo-7B} & \textbf{Qwen-7B} & \textbf{Llama-8B} \\
			\midrule
	...
	\subsubsection{Ablation instruction prompts}
